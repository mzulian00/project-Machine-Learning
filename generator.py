...
class Generator(nn.Module):
	def __init__(self):
		...
		self.stack = nn.Sequential(
			nn.Conv2d(4, FEATURES_G, 5, 1, 2),
			nn.ReLU(inplace=True),
			nn.MaxPool2d(2, 2),
			# 64x64

			nn.Conv2d(FEATURES_G, FEATURES_G*2, 5, 1, 2),
			nn.ReLU(inplace=True),
			nn.MaxPool2d(2, 2),
			# 32x32

			nn.Conv2d(FEATURES_G*2, FEATURES_G*4, 3, 1, 1),
			nn.ReLU(inplace=True),
			nn.MaxPool2d(2, 2),
			# 16x16

			nn.Conv2d(FEATURES_G*4, FEATURES_G*8, 3, 1, 1),
			nn.ReLU(inplace=True),
			nn.MaxPool2d(2, 2),
			# 8x8

			nn.Conv2d(FEATURES_G*8, FEATURES_G*16, 3, 1, 1),
			nn.ReLU(inplace=True),
			nn.MaxPool2d(2, 2),
			# 4x4
